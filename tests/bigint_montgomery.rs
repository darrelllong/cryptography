...
fn rng() -> CtrDrbgAes256 {
    CtrDrbgAes256::new(&[0xa7; 48])
}

fn random_limb(rng: &mut CtrDrbgAes256) -> u64 {
    let mut bytes = [0u8; 8];
...
fn structured_limb(rng: &mut CtrDrbgAes256) -> u64 {
...
fn structured_biguint(words: usize, rng: &mut CtrDrbgAes256) -> BigUint {
    let limbs: Vec<u64> = (0..words).map(|_| structured_limb(rng)).collect();
    from_limbs(&limbs)
}

fn structured_odd_modulus(words: usize, rng: &mut CtrDrbgAes256) -> BigUint {
