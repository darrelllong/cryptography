//! Compare x86 GHASH alternative output against reference outputs.
//!
//! Usage:
//!   cargo run --release --bin compare_ghash --manifest-path fast/x86/x86-alt/Cargo.toml -- [vectors]

use std::hint::black_box;
use std::time::Instant;

use x86_alt::ghash_x86::GhashX86;
...
fn run_correctness(vectors: usize) -> Result<(), String> {
    if !GhashX86::is_supported() {
        return Err("x86 PCLMULQDQ path is not available on this machine".to_string());
    }
    ...
    for i in 0..vectors {
        ...
        let hw = GhashX86::mul(x, y).map_err(|_| "failed to run x86 GHASH".to_string())?;
...
fn run_microbench() -> Result<(), String> {
    ...
    for _ in 0..ITERS {
        ...
        acc_hw ^= GhashX86::mul(x, y).map_err(|_| "x86 GHASH failed".to_string())?;
    }
    ...
    println!(
        "microbench GHASH mul: x86_alt={:.2} Mops/s ct_ref={:.2} Mops/s vt_ref={:.2} Mops/s speedup_vs_ct={:.2}x speedup_vs_vt={:.2}x",
        hw_mops,
        ct_mops,
        vt_mops,
