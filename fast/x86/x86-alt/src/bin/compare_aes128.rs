//! Compare x86 AES-128 alternative output against baseline output.
//!
//! Usage:
//!   cargo run --release --bin compare_aes128 --manifest-path fast/x86/x86-alt/Cargo.toml -- [vectors]

use std::time::Instant;

use cryptography::{Aes128, BlockCipher};
use x86_alt::aes128_x86::Aes128X86;
...
fn run_microbench() -> Result<(), String> {
    ...
    println!(
        "microbench AES-128 encrypt 1MiB: x86_alt={:.2} MiB/s baseline={:.2} MiB/s speedup={:.2}x",
        x86_mibs,
        base_mibs,
        if base_mibs == 0.0 {
            0.0
