//! Compare Apple-Silicon AES-128 alternative output against baseline output.
//!
//! Usage:
//!   cargo run --release --bin compare_aes128 --manifest-path fast/Apple-Silicon/aarch64-alt/Cargo.toml -- [vectors]

use std::time::Instant;

use aarch64_alt::aes128_armv8::Aes128Armv8;
...
fn run_microbench() -> Result<(), String> {
    ...
    println!(
        "microbench AES-128 encrypt 1MiB: arm_alt={:.2} MiB/s baseline={:.2} MiB/s speedup={:.2}x",
        arm_mibs,
        base_mibs,
        if base_mibs == 0.0 {
            0.0
