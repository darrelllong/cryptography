...
const fn build_nibble_anf(table: &[u8; 16]) -> [u16; 4] {
    let mut out = [0u16; 4];
...
const SBOX_ANF: [u16; 4] = build_nibble_anf(&SBOX);
const INV_SBOX_ANF: [u16; 4] = build_nibble_anf(&INV_SBOX);

#[inline(always)]
fn subset_mask4(x: u8) -> u16 {
    let mut mask = 1u16;
...
fn sbox_ct_nibble(input: u8) -> u8 {
    ...
    let mut bit = 0usize;
    while bit < 4 {
        out |= parity16(active & SBOX_ANF[bit]) << bit;
        bit += 1;
    }
    out
}
...
fn inv_sbox_ct_nibble(input: u8) -> u8 {
    ...
    let mut bit = 0usize;
    while bit < 4 {
        out |= parity16(active & INV_SBOX_ANF[bit]) << bit;
        bit += 1;
    }
    out
}
...
#[inline(always)]
fn sbox_layer(mut state: u64) -> u64 {
    let mut out = 0u64;
    let mut i = 0usize;
    while i < 16 {
...
#[inline(always)]
fn inv_sbox_layer(mut state: u64) -> u64 {
    let mut out = 0u64;
    let mut i = 0usize;
    while i < 16 {
        let nibble = ((state >> (4 * i)) & 0x0f) as usize;
