...
#[inline(always)]
fn ct_lookup_u8_16(table: &[u8; 16], idx: u8) -> u8 {
    let mut out = 0u8;
    let mut i = 0usize;
    while i < 16 {
        let x = (i as u8 ^ idx) as u16;
...
fn q_perm_ct(x: u8, which: usize) -> u8 {
    ...
    let a1 = a0 ^ b0;
    let b1 = a0 ^ ror4(b0) ^ ((a0 << 3) & 0x0f);
    let a2 = ct_lookup_u8_16(t0, a1);
    let b2 = ct_lookup_u8_16(t1, b1);
    let a3 = a2 ^ b2;
    let b3 = a2 ^ ror4(b2) ^ ((a2 << 3) & 0x0f);
    let a4 = ct_lookup_u8_16(t2, a3);
    let b4 = ct_lookup_u8_16(t3, b3);
    (b4 << 4) | a4
}
