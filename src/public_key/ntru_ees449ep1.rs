//! NTRUEncrypt-EES449EP1 implemented in safe, idiomatic Rust from IEEE Std
...
//! - the EES449EP1 parameter set ($N = 449$, $q = 2048$, $p = 3$,
//!   $df = 134$, $dg = 149$, $dm0 = 134$, $db = 128$ bits)
...
// ---- parameter constants (IEEE Std 1363.1-2008 Table 1, EES449EP1) --------
...
/// EES449EP1 public key (`h = 3 · g · f^{-1}` packed as `N` 11-bit
/// coefficients, total 618 bytes).
#[derive(Clone, Eq, PartialEq)]
pub struct NtruEes449Ep1PublicKey {
    bytes: [u8; PUBLIC_KEY_BYTES],
}

/// EES449EP1 private key (`t` packed as 2-bit signed trinary, total 113
/// bytes).
#[derive(Clone, Eq, PartialEq)]
pub struct NtruEes449Ep1PrivateKey {
    ...
    pk: NtruEes449Ep1PublicKey,
}

/// EES449EP1 ciphertext.
#[derive(Clone, Eq, PartialEq)]
pub struct NtruEes449Ep1Ciphertext {
    bytes: [u8; CIPHERTEXT_BYTES],
}

impl NtruEes449Ep1PublicKey {
    #[must_use]
...
impl NtruEes449Ep1PrivateKey {
    ...
    pub fn from_wire_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        let pk = NtruEes449Ep1PublicKey::from_wire_bytes(&bytes[PRIVATE_KEY_BYTES..])?;
        Some(Self {
    ...
    #[must_use]
    pub fn public_key(&self) -> &NtruEes449Ep1PublicKey {
        &self.pk
    }
}

impl NtruEes449Ep1Ciphertext {
    #[must_use]
    pub fn from_wire_bytes(bytes: &[u8]) -> Option<Self> {
...
impl fmt::Debug for NtruEes449Ep1PrivateKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("NtruEes449Ep1PrivateKey(<redacted>)")
    }
}

impl fmt::Debug for NtruEes449Ep1PublicKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NtruEes449Ep1PublicKey").finish()
    }
}

impl fmt::Debug for NtruEes449Ep1Ciphertext {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NtruEes449Ep1Ciphertext").finish()
    }
}

/// Errors returnable by encrypt / decrypt.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum NtruEes449Ep1Error {
...
/// Namespace for EES449EP1 NTRUEncrypt operations.
pub struct NtruEes449Ep1;

impl NtruEes449Ep1 {
    /// Generate a fresh key pair.
    pub fn keygen<R: Csprng>(rng: &mut R) -> (NtruEes449Ep1PublicKey, NtruEes449Ep1PrivateKey) {
        loop {
            ...
            let pk = NtruEes449Ep1PublicKey { bytes: pk_bytes };
            let sk = NtruEes449Ep1PrivateKey {
                t,
                pk: pk.clone(),
            };
            return (pk, sk);
    ...
    pub fn encrypt<R: Csprng>(
        ...
        msg: &[u8],
        rng: &mut R,
    ) -> Result<NtruEes449Ep1Ciphertext, NtruEes449Ep1Error> {
        if msg.len() > MAX_MESSAGE_BYTES {
            return Err(NtruEes449Ep1Error::MessageTooLong);
        }
        let h = pk.poly();
        let mut h_centred = h;
        ...
        loop {
            ...
            return Ok(NtruEes449Ep1Ciphertext { bytes: out });
        }
    }
    ...
    pub fn decrypt(
        sk: &NtruEes449Ep1PrivateKey,
        ct: &NtruEes449Ep1Ciphertext,
    ) -> Result<Vec<u8>, NtruEes449Ep1Error> {
        ...
        let cm = sves_to_bytes(&cmtrin).ok_or(NtruEes449Ep1Error::InvalidCiphertext)?;

        let cb = &cm[..DB_BYTES];
        let cl = cm[DB_BYTES] as usize;
        if cl > MAX_MESSAGE_BYTES {
            return Err(NtruEes449Ep1Error::InvalidCiphertext);
        }
...
mod tests {
    ...
    fn parameter_byte_lengths() {
        assert_eq!(N, 449);
        assert_eq!(PUBLIC_KEY_BYTES, 618);
        assert_eq!(PRIVATE_KEY_BYTES, 113);
        assert_eq!(CIPHERTEXT_BYTES, 618);
    ...
    fn round_trip_empty_and_full_messages() {
        ...
        let (pk, sk) = NtruEes449Ep1::keygen(&mut drbg);
        for &len in &[0usize, 1, 16, 32, MAX_MESSAGE_BYTES] {
            let mut msg = vec![0u8; len];
            drbg.fill_bytes(&mut msg);
            let ct = NtruEes449Ep1::encrypt(&pk, &msg, &mut drbg).expect("encrypt");
            let dec = NtruEes449Ep1::decrypt(&sk, &ct).expect("decrypt");
    ...
    fn rejects_oversize_message() {
        ...
        let (pk, _) = NtruEes449Ep1::keygen(&mut drbg);
        let too_big = vec![0u8; MAX_MESSAGE_BYTES + 1];
        let err = NtruEes449Ep1::encrypt(&pk, &too_big, &mut drbg).unwrap_err();
        assert_eq!(err, NtruEes449Ep1Error::MessageTooLong);
    }
    ...
    fn corrupted_ciphertext_rejected() {
        ...
        bad_bytes[10] ^= 0xff;
        let bad_ct =
            NtruEes449Ep1Ciphertext::from_wire_bytes(&bad_bytes).expect("structural decode");
        match NtruEes449Ep1::decrypt(&sk, &bad_ct) {
            Err(NtruEes449Ep1Error::InvalidCiphertext) => {}
    ...
    fn wire_format_roundtrip_keys_and_ct() {
        ...
        let pk_round = NtruEes449Ep1PublicKey::from_wire_bytes(&pk.to_wire_bytes())
            .expect("pk decode");
        let sk_round = NtruEes449Ep1PrivateKey::from_wire_bytes(&sk.to_wire_bytes())
            .expect("sk decode");
        let ct_round = NtruEes449Ep1Ciphertext::from_wire_bytes(&ct.to_wire_bytes())
            .expect("ct decode");
        ...
        let dec = NtruEes449Ep1::decrypt(&sk_round, &ct_round).expect("decrypt");
        assert_eq!(dec, msg);
    }
}
