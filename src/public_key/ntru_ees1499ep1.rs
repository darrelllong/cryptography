//! NTRUEncrypt-EES1499EP1 implemented in safe, idiomatic Rust from IEEE Std
...
//! - the EES1499EP1 parameter set ($N = 1499$, $q = 2048$, $p = 3$,
//!   $df = 79$, $dg = 499$, $dm0 = 79$, $db = 256$ bits)
...
// ---- parameter constants (IEEE Std 1363.1-2008 Table 1, EES1499EP1) --------
...
const MIN_CALLS_R: usize = 17;
const MIN_CALLS_MASK: usize = 19;
const HLEN: usize = 32; // SHA-256
const PKLEN_BITS: usize = 256;
const PKLEN_BYTES: usize = (PKLEN_BITS + 7) / 8; // 32

const OID: [u8; 3] = [0, 6, 5];

/// Public-key wire length (one S_q polynomial of `N` 11-bit coefficients).
pub const PUBLIC_KEY_BYTES: usize = (N * LOGQ + 7) / 8; // 2062
/// Private-key wire length: trinary `t` packed as 2 bits per coefficient.
pub const PRIVATE_KEY_BYTES: usize = (N * 2 + 7) / 8; // 375
/// Ciphertext wire length (same shape as the public key).
pub const CIPHERTEXT_BYTES: usize = PUBLIC_KEY_BYTES; // 2062

/// Maximum byte length of a plaintext message under EES1499EP1.
///
...
/// EES1499EP1 public key (`h = 3 · g · f^{-1}` packed as `N` 11-bit
/// coefficients, total 2062 bytes).
#[derive(Clone, Eq, PartialEq)]
pub struct NtruEes1499Ep1PublicKey {
    bytes: [u8; PUBLIC_KEY_BYTES],
}

/// EES1499EP1 private key (`t` packed as 2-bit signed trinary, total 375
/// bytes).
#[derive(Clone, Eq, PartialEq)]
pub struct NtruEes1499Ep1PrivateKey {
    ...
    pk: NtruEes1499Ep1PublicKey,
}

/// EES1499EP1 ciphertext.
#[derive(Clone, Eq, PartialEq)]
pub struct NtruEes1499Ep1Ciphertext {
    bytes: [u8; CIPHERTEXT_BYTES],
}

impl NtruEes1499Ep1PublicKey {
    #[must_use]
...
impl NtruEes1499Ep1PrivateKey {
    ...
    pub fn from_wire_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        let pk = NtruEes1499Ep1PublicKey::from_wire_bytes(&bytes[PRIVATE_KEY_BYTES..])?;
        Some(Self {
    ...
    #[must_use]
    pub fn public_key(&self) -> &NtruEes1499Ep1PublicKey {
        &self.pk
    }
}

impl NtruEes1499Ep1Ciphertext {
    #[must_use]
    pub fn from_wire_bytes(bytes: &[u8]) -> Option<Self> {
...
impl fmt::Debug for NtruEes1499Ep1PrivateKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("NtruEes1499Ep1PrivateKey(<redacted>)")
    }
}

impl fmt::Debug for NtruEes1499Ep1PublicKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NtruEes1499Ep1PublicKey").finish()
    }
}

impl fmt::Debug for NtruEes1499Ep1Ciphertext {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NtruEes1499Ep1Ciphertext").finish()
    }
}

/// Errors returnable by encrypt / decrypt.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum NtruEes1499Ep1Error {
...
/// Namespace for EES1499EP1 NTRUEncrypt operations.
pub struct NtruEes1499Ep1;

impl NtruEes1499Ep1 {
    /// Generate a fresh key pair.
    pub fn keygen<R: Csprng>(rng: &mut R) -> (NtruEes1499Ep1PublicKey, NtruEes1499Ep1PrivateKey) {
        loop {
            ...
            let pk = NtruEes1499Ep1PublicKey { bytes: pk_bytes };
            let sk = NtruEes1499Ep1PrivateKey {
                t,
                pk: pk.clone(),
            };
            return (pk, sk);
    ...
    pub fn encrypt<R: Csprng>(
        ...
        msg: &[u8],
        rng: &mut R,
    ) -> Result<NtruEes1499Ep1Ciphertext, NtruEes1499Ep1Error> {
        if msg.len() > MAX_MESSAGE_BYTES {
            return Err(NtruEes1499Ep1Error::MessageTooLong);
        }
        let h = pk.poly();
        let mut h_centred = h;
        ...
        loop {
            ...
            return Ok(NtruEes1499Ep1Ciphertext { bytes: out });
        }
    }
    ...
    pub fn decrypt(
        sk: &NtruEes1499Ep1PrivateKey,
        ct: &NtruEes1499Ep1Ciphertext,
    ) -> Result<Vec<u8>, NtruEes1499Ep1Error> {
        ...
        let cm = sves_to_bytes(&cmtrin).ok_or(NtruEes1499Ep1Error::InvalidCiphertext)?;

        let cb = &cm[..DB_BYTES];
        let cl = cm[DB_BYTES] as usize;
        if cl > MAX_MESSAGE_BYTES {
            return Err(NtruEes1499Ep1Error::InvalidCiphertext);
        }
...
mod tests {
    ...
    fn parameter_byte_lengths() {
        assert_eq!(N, 1499);
        assert_eq!(PUBLIC_KEY_BYTES, 2062);
        assert_eq!(PRIVATE_KEY_BYTES, 375);
        assert_eq!(CIPHERTEXT_BYTES, 2062);
    ...
    fn round_trip_empty_and_full_messages() {
        ...
        let (pk, sk) = NtruEes1499Ep1::keygen(&mut drbg);
        for &len in &[0usize, 1, 16, 32, MAX_MESSAGE_BYTES] {
            let mut msg = vec![0u8; len];
            drbg.fill_bytes(&mut msg);
            let ct = NtruEes1499Ep1::encrypt(&pk, &msg, &mut drbg).expect("encrypt");
            let dec = NtruEes1499Ep1::decrypt(&sk, &ct).expect("decrypt");
    ...
    fn rejects_oversize_message() {
        ...
        let (pk, _) = NtruEes1499Ep1::keygen(&mut drbg);
        let too_big = vec![0u8; MAX_MESSAGE_BYTES + 1];
        let err = NtruEes1499Ep1::encrypt(&pk, &too_big, &mut drbg).unwrap_err();
        assert_eq!(err, NtruEes1499Ep1Error::MessageTooLong);
    }
    ...
    fn corrupted_ciphertext_rejected() {
        ...
        bad_bytes[10] ^= 0xff;
        let bad_ct =
            NtruEes1499Ep1Ciphertext::from_wire_bytes(&bad_bytes).expect("structural decode");
        match NtruEes1499Ep1::decrypt(&sk, &bad_ct) {
            Err(NtruEes1499Ep1Error::InvalidCiphertext) => {}
    ...
    fn wire_format_roundtrip_keys_and_ct() {
        ...
        let pk_round = NtruEes1499Ep1PublicKey::from_wire_bytes(&pk.to_wire_bytes())
            .expect("pk decode");
        let sk_round = NtruEes1499Ep1PrivateKey::from_wire_bytes(&sk.to_wire_bytes())
            .expect("sk decode");
        let ct_round = NtruEes1499Ep1Ciphertext::from_wire_bytes(&ct.to_wire_bytes())
            .expect("ct decode");
        ...
        let dec = NtruEes1499Ep1::decrypt(&sk_round, &ct_round).expect("decrypt");
        assert_eq!(dec, msg);
    }
}
