//! NTRUEncrypt-EES401EP1 implemented in safe, idiomatic Rust from IEEE Std
...
//! - the EES401EP1 parameter set (`N = 401`, `q = 2048`, `p = 3`,
//!   `df = 113`, `dg = 133`, `dm0 = 113`, `db = 112` bits)
...
// ---- parameter constants (IEEE Std 1363.1-2008 Table 1, EES401EP1) --------
...
const MIN_CALLS_R: usize = 32;
const MIN_CALLS_MASK: usize = 9;
const HLEN: usize = 20; // SHA-1
const PKLEN_BITS: usize = 114;
const PKLEN_BYTES: usize = (PKLEN_BITS + 7) / 8; // 15

const OID: [u8; 3] = [0, 2, 4];

/// Public-key wire length (one S_q polynomial of `N` 11-bit coefficients).
pub const PUBLIC_KEY_BYTES: usize = (N * LOGQ + 7) / 8; // 552
/// Private-key wire length: trinary `t` packed as 2 bits per coefficient.
pub const PRIVATE_KEY_BYTES: usize = (N * 2 + 7) / 8; // 101
/// Ciphertext wire length (same shape as the public key).
pub const CIPHERTEXT_BYTES: usize = PUBLIC_KEY_BYTES; // 552

/// Maximum byte length of a plaintext message under EES401EP1.
///
...
/// EES401EP1 public key (`h = 3 · g · f^{-1}` packed as `N` 11-bit
/// coefficients, total 552 bytes).
#[derive(Clone, Eq, PartialEq)]
pub struct NtruEes401Ep1PublicKey {
    bytes: [u8; PUBLIC_KEY_BYTES],
}

/// EES401EP1 private key (`t` packed as 2-bit signed trinary, total 101
/// bytes).
#[derive(Clone, Eq, PartialEq)]
pub struct NtruEes401Ep1PrivateKey {
    ...
    pk: NtruEes401Ep1PublicKey,
}

/// EES401EP1 ciphertext.
#[derive(Clone, Eq, PartialEq)]
pub struct NtruEes401Ep1Ciphertext {
    bytes: [u8; CIPHERTEXT_BYTES],
}

impl NtruEes401Ep1PublicKey {
    #[must_use]
...
impl NtruEes401Ep1PrivateKey {
    ...
    pub fn from_wire_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        let pk = NtruEes401Ep1PublicKey::from_wire_bytes(&bytes[PRIVATE_KEY_BYTES..])?;
        Some(Self {
    ...
    #[must_use]
    pub fn public_key(&self) -> &NtruEes401Ep1PublicKey {
        &self.pk
    }
}

impl NtruEes401Ep1Ciphertext {
    #[must_use]
    pub fn from_wire_bytes(bytes: &[u8]) -> Option<Self> {
...
impl fmt::Debug for NtruEes401Ep1PrivateKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("NtruEes401Ep1PrivateKey(<redacted>)")
    }
}

impl fmt::Debug for NtruEes401Ep1PublicKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NtruEes401Ep1PublicKey").finish()
    }
}

impl fmt::Debug for NtruEes401Ep1Ciphertext {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NtruEes401Ep1Ciphertext").finish()
    }
}

/// Errors returnable by encrypt / decrypt.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum NtruEes401Ep1Error {
...
/// Namespace for EES401EP1 NTRUEncrypt operations.
pub struct NtruEes401Ep1;

impl NtruEes401Ep1 {
    /// Generate a fresh key pair.
    pub fn keygen<R: Csprng>(rng: &mut R) -> (NtruEes401Ep1PublicKey, NtruEes401Ep1PrivateKey) {
        loop {
            ...
            let pk = NtruEes401Ep1PublicKey { bytes: pk_bytes };
            let sk = NtruEes401Ep1PrivateKey {
                t,
                pk: pk.clone(),
            };
            return (pk, sk);
    ...
    pub fn encrypt<R: Csprng>(
        ...
        msg: &[u8],
        rng: &mut R,
    ) -> Result<NtruEes401Ep1Ciphertext, NtruEes401Ep1Error> {
        if msg.len() > MAX_MESSAGE_BYTES {
            return Err(NtruEes401Ep1Error::MessageTooLong);
        }
        let h = pk.poly();
        let mut h_centred = h;
        ...
        loop {
            ...
            return Ok(NtruEes401Ep1Ciphertext { bytes: out });
        }
    }
    ...
    pub fn decrypt(
        sk: &NtruEes401Ep1PrivateKey,
        ct: &NtruEes401Ep1Ciphertext,
    ) -> Result<Vec<u8>, NtruEes401Ep1Error> {
        ...
        let cm = sves_to_bytes(&cmtrin).ok_or(NtruEes401Ep1Error::InvalidCiphertext)?;

        let cb = &cm[..DB_BYTES];
        let cl = cm[DB_BYTES] as usize;
        if cl > MAX_MESSAGE_BYTES {
            return Err(NtruEes401Ep1Error::InvalidCiphertext);
        }
...
mod tests {
    ...
    fn parameter_byte_lengths() {
        assert_eq!(N, 401);
        assert_eq!(PUBLIC_KEY_BYTES, 552);
        assert_eq!(PRIVATE_KEY_BYTES, 101);
        assert_eq!(CIPHERTEXT_BYTES, 552);
    ...
    fn round_trip_empty_and_full_messages() {
        ...
        let (pk, sk) = NtruEes401Ep1::keygen(&mut drbg);
        for &len in &[0usize, 1, 16, 32, MAX_MESSAGE_BYTES] {
            let mut msg = vec![0u8; len];
            drbg.fill_bytes(&mut msg);
            let ct = NtruEes401Ep1::encrypt(&pk, &msg, &mut drbg).expect("encrypt");
            let dec = NtruEes401Ep1::decrypt(&sk, &ct).expect("decrypt");
    ...
    fn rejects_oversize_message() {
        ...
        let (pk, _) = NtruEes401Ep1::keygen(&mut drbg);
        let too_big = vec![0u8; MAX_MESSAGE_BYTES + 1];
        let err = NtruEes401Ep1::encrypt(&pk, &too_big, &mut drbg).unwrap_err();
        assert_eq!(err, NtruEes401Ep1Error::MessageTooLong);
    }
    ...
    fn corrupted_ciphertext_rejected() {
        ...
        bad_bytes[10] ^= 0xff;
        let bad_ct =
            NtruEes401Ep1Ciphertext::from_wire_bytes(&bad_bytes).expect("structural decode");
        match NtruEes401Ep1::decrypt(&sk, &bad_ct) {
            Err(NtruEes401Ep1Error::InvalidCiphertext) => {}
    ...
    fn wire_format_roundtrip_keys_and_ct() {
        ...
        let pk_round = NtruEes401Ep1PublicKey::from_wire_bytes(&pk.to_wire_bytes())
            .expect("pk decode");
        let sk_round = NtruEes401Ep1PrivateKey::from_wire_bytes(&sk.to_wire_bytes())
            .expect("sk decode");
        let ct_round = NtruEes401Ep1Ciphertext::from_wire_bytes(&ct.to_wire_bytes())
            .expect("ct decode");
        ...
        let dec = NtruEes401Ep1::decrypt(&sk_round, &ct_round).expect("decrypt");
        assert_eq!(dec, msg);
    }
}
